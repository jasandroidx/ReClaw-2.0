...
class VectorStore:
    ...
    def search(
        ...
        for i, (doc_id, doc_text, meta, distance) in enumerate(zip(ids, docs, metas, distances)):
            ...
            note_status = str((meta or {}).get("note_status") or "").strip().lower()
            if note_status in {"superseded", "trash", "skip", "retired"}:
                continue
    ...
    def _chunk_to_metadata(chunk: DocumentChunk, doc_meta: IngestedDocument) -> dict:
        ...
        raw = {
            ...
            "note_status": (doc_meta.frontmatter or {}).get("status") or "",
