# core/oracle_mcp.py
"""
OculaiMCP Singleton Orchestrator — consolidated production edition.
...
class OculaiMCP:
    _instance: Optional["OculaiMCP"] = None
...
# --- Legacy Oculai MCP shims (ingest.py / stdio server compatibility) ---

OCULAI_PATH = Path("/root/obsidian_vault/Ravenstack/RAVENSTACK-OCULAI.md")


def load_oculai_section(section: str) -> str:
    if not OCULAI_PATH.exists():
        return "Oculai not found — SOT violation."
    content = OCULAI_PATH.read_text(encoding="utf-8")
    match = re.search(rf"## {re.escape(section)}.*?(?=## |$)", content, re.DOTALL | re.IGNORECASE)
    return match.group(0).strip() if match else f"Section '{section}' not found in Oculai."


def query_oculai(query: str) -> Dict[str, Any]:
    if "save" in query.lower() or "rules" in query.lower():
        section = load_oculai_section("Oculai Bible — Complete Reference")
    elif "mcp" in query.lower() or "manager" in query.lower():
        section = load_oculai_section("MCP Server & KnowledgeManager")
    else:
        section = load_oculai_section("Oculai Bible — Complete Reference")
    return {"status": "success", "result": section, "provenance": "RAVENSTACK-OCULAI.md"}


def validate_against_oculai(plan_or_content: str) -> Dict[str, Any]:
    ...
    if not any(tag in plan_or_content for tag in ["status:", "potential_for:", "tags:", "provenance:"]):
        violations.append("Missing required frontmatter per Oculai tagging standards")
    status = "pass" if not violations else "fail"
    return {
        "status": status,
        "violations": violations,
        "recommendation": "Fix and re-validate via MCP" if violations else "Approved per Oculai",
        "provenance": "RAVENSTACK-OCULAI.md validation",
    }
...
def ingest_document(source: str, distill: bool = True) -> Dict[str, Any]:
    ...
    settings.validate_oracle()
    distilled = (
        f"# Distilled from {source}\n\nPer Oculai rules: Consult full bible in RAVENSTACK-OCULAI.md.\n"
...
def _legacy_stdio_main():
    for line in sys.stdin:
        try:
            ...
            if tool == "query_oculai":
                result = query_oculai(args.get("query", ""))
            elif tool == "validate_against_oculai":
                result = validate_against_oculai(args.get("content", ""))
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1] == "--test":
        print("OculaiMCP consolidated test:")
