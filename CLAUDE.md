...
**Claude: Jason left you a direct note. Read `FOR-CLAUDE.md` in this repo root before any ops/MCP/gateway work.**
...
- **Port 8100 is reclaw-platform only** (`reclaw-mcp-bridge` → `scripts/run-reclaw-mcp-bridge.sh`). Do **not** bind another process there. Do **not** `fuser -k 8100`, `kill` the pid on 8100, or `enable` `raven-mcp-bridge` (masked). Keep MCP is **8111**. If MCP is 404/failed, read `/root/obsidian_vault/Ravenstack/ops/incidents/INDEX.md` before mutating. Incident: `2026-08-18-mcp-8100-port-thief`.
...
- Binding a new MCP (SSE or otherwise) to `:8100`, or killing whatever already listens there so “your” server can start
