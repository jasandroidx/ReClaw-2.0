...
def _detect_ts_ip() -> str:
    """This host's tailnet IPv4. Never fall back to the old Hetzner 100.108.130.82."""
    env = (os.environ.get("TAILSCALE_IP") or "").strip()
    if env:
        return env
    try:
        ...
        pass
    return "100.85.152.115"


_TS_IP = _detect_ts_ip()
...
mcp = FastMCP(
    ...
    # process must not bind the old Hetzner address 100.108.130.82.
...
@mcp.tool()
def read_oculai(section: str = "") -> str:
    """Read RAVENSTACK-OCULAI.md (optionally one section by heading)."""
    if section:
        sec = _km().get_section("RAVENSTACK-OCULAI.md", section)
        return f"## {sec.title}\n\n{sec.content}"
    return _km().get("RAVENSTACK-OCULAI.md")[:12000]
...
def ingest_to_ravenstack(source: str, content_or_path: str) -> str:
    """Ingest distilled content into Ravenstack backlog (OCULAI rules)."""
...
def _project_sitrep_sync() -> str:
    ...
    if not public_url:
        public_url = "https://openclaw.tail20a090.ts.net/rk7m2q9x/mcp"
    public_code = ""
    if public_url:
    ...
    # Public plane: Funnel :443 /rk7m2q9x — GET /mcp expects HTTP 406 (streamable-http).
    ...
    is_funnel = "/rk7m2q9x" in (public_url or "") and "openclaw.tail20a090.ts.net" in (
        public_url or ""
    )
    if is_funnel:
    ...
    oculai_ok = (kp / "RAVENSTACK-OCULAI.md").is_file()
    ...
    if not oculai_ok:
        ...
        actions.append("Restore Ravenstack/RAVENSTACK-OCULAI.md in the vault")
    if not mcp_doc_ok:
