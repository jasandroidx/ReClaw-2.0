...
def ingest_document(source: str, content_or_path: str, auto_categorize: bool = True) -> str:
    """Distill and ingest a document into Ravenstack backlog per OCULAI rules."""
...
def list_knowledge_topics() -> str:
    """List Ravenstack knowledge files and OCULAI anchors."""
...
def read_oculai(section: str = "") -> str:
    """Read RAVENSTACK-OCULAI.md or a specific section heading."""
    if section:
        sec = _km().get_section("RAVENSTACK-OCULAI.md", section)
        return f"## {sec.title}\n\n{sec.content}"
    return _km().get("RAVENSTACK-OCULAI.md")[:8000]
...
Reads: project_sitrep (FULL status), query_knowledge, read_oculai, stack_health,
