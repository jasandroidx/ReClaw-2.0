...
def latest_job():
    ...
    runs = get_sorted_glob_by_mtime(settings.runs_dir, "*.json", reverse=True)
    if not runs:
...
def list_packages(limit: int = 20):
    ...
    items = []
    for p in get_sorted_glob_by_mtime(settings.runs_dir, "*.json", reverse=True, limit=limit):
        try:
...
def get_state():
    ...
    if sess_root.exists():
        sorted_sessions = get_sorted_files_by_mtime(sess_root, reverse=True, is_dir=True, limit=5)
...
def list_sessions(limit: int = 20):
    ...
    if not sess_root.exists():
        return {"sessions": []}
    items = []
    for p in get_sorted_files_by_mtime(sess_root, reverse=True, is_dir=True, limit=limit):
