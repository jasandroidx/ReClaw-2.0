...
# Session 1 Clawsmith Visual Dashboard Upgrades (per approved plan)
# Aligns with ravenstack: upgrades existing compiler for multi-room floor, Named Event Bus, Total-ReClaw, sprites, passive income connections.
...
class AgentDesk(BaseModel):
    id: str
    role: str
    spriteAssetId: str  # e.g. "purple_grant_watcher", "green_clothed_aggregator"
    tools: List[str]
    anchor_coords: Dict[str, int] = {"x": 220, "y": 180}  # for dashboard anchoring

class CellBlueprint(BaseModel):
    room_id: str
    theme: str  # "Grant Hall Dungeon" or "Hacker Den Terminal"
    chief_agent: str
    workers: List[AgentDesk]
    approval_gates: List[str]
    revenue_triggers: List[str] = []
    sprite_map: Dict[str, str] = {}

class ClawsmithCompiler:
    """Upgraded compiler per spec for Session 1. Integrates with existing Clawforge."""
    def __init__(self):
        self.workspace = Path("/root/.openclaw/workspace/rooms")

    def prune_overengineered_workers(self, cell: CellBlueprint) -> CellBlueprint:
        """Per spec: consolidate scrapers/parsers into Data Acquisition specialist. Score by income potential."""
        scrapers = [w for w in cell.workers if "scraper" in w.role.lower() or "parser" in w.role.lower()]
        if len(scrapers) > 1:
            consolidated = AgentDesk(
                id="data_processor",
                role="Data Acquisition and Parsing Specialist",
                spriteAssetId="green_clothed_aggregator",
                tools=list(set([t for w in scrapers for t in w.tools])),
                anchor_coords={"x": 150, "y": 150}
            )
            cell.workers = [w for w in cell.workers if w not in scrapers]
            cell.workers.append(consolidated)
            cell.revenue_triggers.append("bundled_data_service")
        return cell

    def apply_visual_theme(self, cell: CellBlueprint) -> CellBlueprint:
        """Per spec: theme based on roomId for passive income rooms."""
        if any(k in cell.room_id.lower() for k in ["grant", "fund", "hall"]):
            cell.theme = "Grant Hall Dungeon"
            cell.revenue_triggers.append("grant_alert_49mo")
        else:
            cell.theme = "Hacker Den Terminal"
        return cell

    def init_total_reclaw_memory(self, room_path: Path):
        """Upgrade for Total-ReClaw: sqlite-vec + FTS5, scoring formula, consolidation."""
        db_path = room_path / "memory.db"
        conn = sqlite3.connect(db_path)
        conn.execute("PRAGMA journal_mode=WAL;")
        try:
            conn.execute("SELECT vec_version();")  # Load vec extension if available
            conn.execute("""
            ...
            """)
            logger.info("sqlite-vec extension loaded for Total-ReClaw. *CLANG*")
        except:
            logger.info("Fallback to FTS5 for memory (vec not available).")
            conn.execute("CREATE VIRTUAL TABLE IF NOT EXISTS memories USING fts5(content, timestamp, importance, trust);")
        conn.execute("""
        ...
        conn.commit()
        conn.close()
        logger.info(f"Total-ReClaw memory initialized at {db_path} with cosine*decay*rank*boost scoring and 360min consolidation.")

    def emit_event(self, event_type: str, payload: Dict):
        """Named Event Bus per spec (REQUIRED). Emits JSON frames to WS/gateway for dashboard sprites."""
        frame = {"type": event_type, **payload}
        # In production, send to ws://127.0.0.1:18789 named bus
        print(f"[NAMED_EVENT_BUS] {json.dumps(frame)}")  # For dashboard WS listener
        if "hitl:pending" in event_type:
            print("Visual: glowing scroll overlay triggered for approval.")
        elif "boss:offline" in event_type:
            print("Dashboard: full grayscale + pause.")
        logger.info(f"Emitted event: {event_type} for room {payload.get('room', 'unknown')}")

    def compile_goal_to_cell(self, goal: str) -> CellBlueprint:
        """Main compiler per spec TS interface + prune + theme + income. Creates room."""
        # LLM or rules for analysis (fallback to existing template logic)
        raw_structure = self._analyze_goal_requirements(goal)  # Uses existing LLM
        optimized = self.prune_overengineered_workers(raw_structure)
        themed = self.apply_visual_theme(optimized)
        if "grant" in goal.lower():
            themed.revenue_triggers.append("auto_content_episode")  # Income connection
        room_path = self.workspace / themed.room_id
        room_path.mkdir(parents=True, exist_ok=True)
        (room_path / "SOUL.md").write_text(f"""---
name: {themed.room_id}
description: Persistent {themed.theme} cell. Chief: {themed.chief_agent}. Visual dashboard + passive income.
...
**Immutable Rules**
- 3-revision cap. trust="unverified" on injected. Provenance + cosine*decay**elapsed*rank*boost scoring.
- Named Event Bus frames for sprites/states. SYSTEM_PAUSED check.
- Failure modes mitigated (see plan). Every action advances MAIN GOAL: passive income.
*CLANG*
""")
        self.init_total_reclaw_memory(room_path)
        self.emit_event("cell.created", {"room": themed.room_id, "theme": themed.theme, "revenue": themed.revenue_triggers})
        logger.info(f"Cell forged: {themed.room_id} (*CLANG* visual desk sprites anchored, income wired).")
        return themed

    def _analyze_goal_requirements(self, goal: str) -> CellBlueprint:
        """Dynamic via LLM or template. Matches existing _call_llm."""
        # Stub for now; integrates with existing prompt/LLM
        return CellBlueprint(
            room_id="grant_hall_dungeon",
            theme="Grant Hall Dungeon",
            chief_agent="grant_watcher",
            workers=[
                AgentDesk(id="grant_scanner", role="Grant/RFP Scanner", spriteAssetId="purple_grant_watcher", tools=["grant_api", "maps"], anchor_coords={"x": 220, "y": 180}),
                AgentDesk(id="compliance_dive", role="Silent Auditor", spriteAssetId="red_auditor", tools=["audit"], anchor_coords={"x": 300, "y": 120})
            ],
            approval_gates=["write_package", "publish_alert"],
            revenue_triggers=["grant_alert_49mo"],
            sprite_map={"central": "purple_grant_watcher"}
        )

if __name__ == "__main__":
    # Existing argparse + new --cell mode for Session 1
    import argparse
    parser = argparse.ArgumentParser(description="Clawsmith Visual Dashboard Compiler *CLANG*")
    parser.add_argument("--goal", type=str, required=True, help="Plain-English goal for cell")
    parser.add_argument("--cell", action="store_true", help="Use new CellBlueprint compiler for visual floor")
    args = parser.parse_args()
    if args.cell:
        compiler = ClawsmithCompiler()
        blueprint = compiler.compile_goal_to_cell(args.goal)
        print(blueprint.model_dump_json(indent=2))
    else:
        # Existing old logic
        print("Legacy mode. Use --cell for Session 1 Visual Dashboard.")
